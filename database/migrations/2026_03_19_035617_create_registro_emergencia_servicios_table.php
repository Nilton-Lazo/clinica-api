...
return new class extends Migration
{
    ...
    {
        Schema::create('registro_emergencia_servicios', function (Blueprint $table) {
            $table->id();

            $table->foreignId('registro_emergencia_id')
                ->constrained('registro_emergencia')
                ->cascadeOnDelete();
            ...
            $table->string('estado_facturacion', 20)->default('PENDIENTE');

            $table->timestamps();

            $table->index(['registro_emergencia_id']);
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('registro_emergencia_servicios');
    }
};
